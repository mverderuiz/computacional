...
int main (void)
{
    ...
    ofstream planpos("prueba.txt");
    //Variables utilizados para el calculo de periodos
    float periodos[Np];
    int nvueltas[Np];
    double aux[Np], cociente;
    //Variables utilizadas para el calulo de energia
    double enerpla[8];
    double modvel, modpos;
    ofstream energia("energias.txt");
    ...
    for (k=0;k<9;k++)
    {
        ...
        planetas[1][0]=0.38*0.8; //Posicion incial de Mercurio
        planetas[1][3]=48.92E3; //Velocidad inical de Mercurio
        planetas[2][0]=0.72*1.1; //Posicion incial de Venus
        planetas[2][3]=35.02E3; //Velocidad inical de Venus
        planetas[3][0]=1.*1.4; //Posicion incial de La Tierra
        planetas[3][3]=29.78E3; //Velocidad inical de La Tierra
        planetas[4][0]=1.52*1.2; //Posicion incial de Marte
        planetas[4][3]=24.07E3; //Velocidad inical de Marte
        planetas[5][0]=5.21*1.2; //Posicion incial de Jupiter
        planetas[5][3]=13.05E3; //Velocidad inical de Jupiter
        planetas[6][0]=9.54*1.15; //Posicion incial de Saturno
        planetas[6][3]=9.64E3; //Velocidad inical de Saturno
        planetas[7][0]=19.18*0.95; //Posicion incial de Urano
        planetas[7][3]=6.81E3; //Velocidad inical de Urano
        planetas[8][0]=30.11*0.7; //Posicion incial de Neptuno
    ...
    //Calculamos las energias inciales de los planetas;
    energia << t ;
            for(k=0; k<8; k++)
            {
                modvel=planetas[k+1][2]*planetas[k+1][2]+planetas[k+1][3]*planetas[k+1][3];
                modpos=sqrt( planetas[k+1][0]*planetas[k+1][0]+planetas[k+1][1]*planetas[k+1][1] );
                enerpla[k]=masas[k+1]*modvel/2.+masas[0]*masas[k+1]/modpos;    
                energia << "\t" << enerpla[k] << "\t";    
            }
            energia << endl;  
    ...
    for(i=0;i<10000  ;i++)
    {
        ...
        for (z=0; z<8; z++)
        {
            ...
            if ( (planetas[z+1][0]>0) && (cociente<0) )
            {
                periodos[z]= t;
                nvueltas[z] ++;
            }       
    ...
    for (i=0; i<8; i++)
    {
        periodos[i]=(periodos[i]/nvueltas[i])*58.1;
