...
int main (void)
{
    ...
    double planetas[Np][6];
    //Variable del tiempo y paso
    float t=0.;
    float h=0.01;
    //Archivo donde se van a escribir las posiciones
    ofstream planpos("posicion_eliptica.txt");
    //Variables utilizados para el calculo de periodos
    float periodos[Np];
    int nvueltas[Np];
    double aux[Np], cociente;
    ...
    for (k=0;k<Np;k++)
    {
        ...
        planetas[1][0]=69.8; //Posicion incial de Mercurio
        planetas[1][3]=39.E3; //Velocidad inical de Mercurio
        planetas[2][0]=108.; //Posicion incial de Venus
        planetas[2][3]=34.8E3; //Velocidad inical de Venus
        planetas[3][0]=152.; //Posicion incial de La Tierra
        planetas[3][3]=29.2E3; //Velocidad inical de La Tierra
        planetas[4][0]=249.; //Posicion incial de Marte
        planetas[4][3]=21.7E3; //Velocidad inical de Marte
        planetas[5][0]=816.; //Posicion incial de Jupiter
        planetas[5][3]=12.44E3; //Velocidad inical de Jupiter
        planetas[6][0]=1500.; //Posicion incial de Saturno
        planetas[6][3]=9.09E3; //Velocidad inical de Saturno
        planetas[7][0]=3000.; //Posicion incial de Urano
        planetas[7][3]=6.49E3; //Velocidad inical de Urano
        planetas[8][0]=4540.; //Posicion incial de Neptuno
        planetas[8][3]=5.37E3; //Velocidad inical de Neptuno
    ...
    for (k=0;k<Np;k++)
    {
        ...
        planetas[k][0]=planetas[k][0]/(1.496E2);
    }
    ...
    for(k=0;k<9;k++) planpos << planetas[k][0] <<  ", " << planetas[k][1] << endl; planpos << endl;

    //Inicializamos los periodos de los planetas y el num de vueltas a cero
    for (z=0; z<Np; z++)
    {
        periodos[z]=0.;
        nvueltas[z]=0;
    ...
    for(i=1;i<10000;i++)
    {
        ...
        if (i%5==0)
        {
            for(k=0;k<8;k++)  planpos << planetas[k][0] <<  ", " << planetas[k][1] << endl;
            planpos << endl;
        }
        //Calculo de periodos.
        for (z=0; z<8; z++)
        {
            cociente=planetas[z+1][1]/aux[z];
            //Comprobamos si el planeta ha dado media vuelta
            if ( (planetas[z+1][0]>0) && (cociente<0) && (nvueltas[z]<50))
            {
                periodos[z]= t;
                nvueltas[z] ++;
            }       
            aux[z]=planetas[z+1][1];   
        } 
    }    
    ...
    for (i=0; i<8; i++)
    {
        periodos[i]=(periodos[i]/nvueltas[i])*58.1/365.;
        cout << periodos[i] << "\t" << nvueltas[i] << endl;
    }
